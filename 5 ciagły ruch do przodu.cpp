...
void Custom::LowCmdWrite()
{
    ...
    if (mode == 1)
    {
        ...
        if (step == 0) {
            ...
            // Jeśli robot już stoi (przyjmijmy sprawdzenie jednego przegubu z tolerancją)
            if (std::abs(low_state.motor_state()[2].q() - stand_up_joint_pos[2]) < tolerance)
            {
                step = 1;
                    std::cout << "osiagnięto góre \n";
            }
        }
        else if (step == 1)  // Po 1 sekundzie od wstania
        {
            for (int i = 0; i < 12; i++)
            {
            ...
            if (std::abs(low_state.motor_state()[2].q() - stand_down_joint_pos[2]) < tolerance)
            {
                step = 2;  // Reset sekwencji
          std::cout << "osiagnięto dół \n";
            }
        }
    }
else if (mode == 2)
